# lib/src/runtime/quantum_test_engine.dart

This file is covered by the central YAML-driven test engine.

## Purpose

Central YAML-driven test orchestration, manifest discovery, result aggregation, and compact reporting.

## Test contract

The matching YAML manifest lives at `docs_tests/yaml/by-file/src/runtime/quantum_test_engine.yaml` and expands into strict row-level cases with stable ids, inputs, expected outcomes, and compact reporting.
...
public API, manifest loading, result aggregation, update detection, compatibility, error handling
