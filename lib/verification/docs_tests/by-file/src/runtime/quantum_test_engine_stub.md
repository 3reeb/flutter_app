# lib/src/runtime/quantum_test_engine_stub.dart

This file is covered by the central YAML-driven test engine.

## Purpose

Unsupported-platform stub that keeps the package analyzable on environments without dart:io.

## Test contract

The matching YAML manifest lives at `docs_tests/yaml/by-file/src/runtime/quantum_test_engine_stub.yaml` and expands into strict row-level cases with stable ids, inputs, expected outcomes, and compact reporting.
...
compatibility, unsupported platform behavior, compile safety, guardrails
