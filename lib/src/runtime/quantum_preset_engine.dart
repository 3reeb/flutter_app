/*
 * ============================================================================
 * File: quantum_preset_engine.dart
 * 
 * Description:
 * Implements a hybrid AOT template engine (The Omega Compiler) for Quantum. 
 * It manages lazy inheritance, merging, and caching of SDUI presets, converting 
 * raw JSON/YAML definitions into highly optimized PresetDef structures 
 * containing binary layouts and pre-computed variants.
 * 
 * Key Components:
 * - QPresetEngine: Static engine handling definition, resolution, and caching.
 * - PresetDef: The compiled, immutable representation of a template.
 * - QPresetContext: The "God-Mode" context unifying slots, layouts, and state.
 * 
 * Dependencies/Relationships:
 * Deeply integrated with quantum_layout.dart and quantum_omni_registry.dart 
 * to provide reusable component templates across the framework.
 * 
 * Notes:
 * Uses a two-phase compilation (pending -> resolved) to safely handle deep 
...
class _PendingPresetDef {
  ...
  final QNativePresetBuilder? nativeBuilder;
  final String? extendsAlias;
  final bool mergeWithBase;

  const _PendingPresetDef({
    required this.alias,
    this.layout,
...
abstract final class QPresetEngine {
  /// Fully resolved, cached template definitions (keyed by alias.hashCode).
  static final Map<int, PresetDef> _registry = <int, PresetDef>{};

  /// Raw definitions waiting for lazy inheritance resolution.
  static final Map<int, _PendingPresetDef> _pending =
      <int, _PendingPresetDef>{};
  ...
  static PresetDef? getDef(String alias) {
    final int key = alias.hashCode;
    final PresetDef? cached = _registry[key];
    if (cached != null) return cached;

    final _PendingPresetDef? pending = _pending[key];
    ...
    try {
      ...
      final PresetDef? base = (pending.extendsAlias != null &&
      ...
      final QNativePresetBuilder? resolvedNativeBuilder =
      ...
      final PresetDef resolved = PresetDef(
        alias: pending.alias,
  ...
  static void defineMany(
    ...
    Map<String, QNativePresetBuilder>? nativeBuilders,
  }) {
  ...
  static void define(
    ...
    QNativePresetBuilder? nativeBuilder,
    String? extendsAlias,
    bool mergeWithBase = true,
  }) {
    if (_frozen) {
      throw StateError('QPresetEngine is frozen. Call unfreeze() first.');
    }
    ...
    _pending[key.hashCode] = _PendingPresetDef(
      alias: key,
      layout: layout,
  ...
  static void defineVariant(
    ...
    _registry[alias.hashCode] = PresetDef(
      alias: def.alias,
...
typedef QNativePresetBuilder = Widget Function(QPresetContext ctx);
...
/// The God-Mode Context. Unifies Slots, Layouts, Variants, Heroes, and State into a single API.
class QPresetContext {
  final QLContext core;
  final PresetDef def;
  final String instanceId;

  QPresetContext(this.core, this.def, this.instanceId);
