...
class QCryptoTamperException extends QCryptoException {
  const QCryptoTamperException()
      : super(
            'Authentication tag mismatch — blob may be tampered or corrupted.');
}
...
class _PureDartAES256GCM implements QCryptoBackend {
  ...
  Uint8List decryptRaw(
    ...
    final ciphertext = Uint8List.sublistView(
        ciphertextWithTag, 0, ciphertextWithTag.length - 16);
    final receivedTag =
        Uint8List.sublistView(ciphertextWithTag, ciphertextWithTag.length - 16);
...
class QCryptoEngine {
  ...
  Uint8List decrypt(Uint8List ciphertext) {
    ...
    if (ciphertext.length < _nonceLength + _tagLength) {
      throw const QCryptoException(
          'Blob too short to be a valid QEE ciphertext');
    }
  ...
  Future<Uint8List> decryptAsync(Uint8List ciphertext) async {
    ...
    return compute(
        _decryptInIsolate, _DecryptPayload(_derivedKey!, ciphertext));
  }
...
Uint8List _encryptInIsolate(_EncryptPayload payload) {
  ...
  final encrypted = _PureDartAES256GCM.instance
      .encryptRaw(payload.key, nonce, payload.plaintext);
  final out = Uint8List(_nonceLength + encrypted.length);
  out.setAll(0, nonce);
  out.setAll(_nonceLength, encrypted);
  return out;
