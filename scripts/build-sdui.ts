...
function normalizeForOutput<T>(value: T): T {
  ...
  if (typeof obj.toJSON === 'function') {
    return normalizeForOutput(obj.toJSON() as T);
  }
...
async function buildSdui() {
  try {
    ...
    const kernelConfig: any = typeof config.exportAll === 'function' ? config.exportAll() : { ...config };
    
    // Map TypeScript 'templates' array -> Dart 'macros' map
    if (Array.isArray(kernelConfig.templates)) {
      kernelConfig.macros = kernelConfig.macros || {};
      for (const tpl of kernelConfig.templates) {
        if (tpl && tpl.name) {
          kernelConfig.macros[tpl.name] = { type: 'template', ...tpl };
        }
      }
      delete kernelConfig.templates;
    }
